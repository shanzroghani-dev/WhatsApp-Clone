...
class ChatScreenState extends State<ChatScreen> {
  ...
  void _onScroll() {
    if (!_scrollController.hasClients || _isLoadingMore || !_hasMoreMessages) return;
  ...
  void _subscribeToIncomingMessages() {
    _incomingSubscription ??= ChatService.streamMessagesBetween(
      widget.peer.uid,
  ...
  Future<void> _loadMessages({bool scrollToBottom = false}) async {
    ...
    setState(() {
      ...
      if (previousVisibleCount == 0) {
        _visibleCount = messages.length < _pageSize ? messages.length : _pageSize;
      } else {
  ...
  void _loadMoreMessages() {
    ...
    final previousMaxExtent = hasClients ? _scrollController.position.maxScrollExtent : 0.0;

    setState(() {
      _isLoadingMore = true;
    });

    final nextVisibleCount = (_visibleCount + _pageSize).clamp(0, _messages.length);
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final targetOffset = (previousPixels + extentDelta).clamp(0.0, newMaxExtent);
  ...
  void _sendMessage() {
    ...
    unawaited(_sendTextMessageInBackground(
      text: text,
  ...
  Future<void> _sendSelectedMedia() async {
    ...
    try {
      ...
      if (_attachmentCacheDirPath != null && isVideo && thumbnailBytes != null) {
        thumbnailPath = '$_attachmentCacheDirPath/${tempMessageId}_thumbnail.jpg';
      }
      ...
      unawaited(_uploadMediaInBackground(
        file: file,
        tempMessageId: tempMessageId,
        mediaType: mediaType,
        fileName: fileName,
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to prepare media')),
      );
    }
  }
  ...
  Future<String?> _downloadAttachment({
    ...
    try {
      ...
      if (mediaType == 'video') {
        final thumbnailData = await ChatHelpers.generateVideoThumbnail(savePath);
  ...
  Future<void> _onMediaMessageTap({
    ...
    final displayName = (attachment['name'] as String?) ?? (isVideo ? 'Video' : 'Photo');
    ...
    final downloadedPath = await _downloadAttachment(message: message, attachment: attachment);
    if (!mounted) return;
    if (downloadedPath != null) {
      _openMediaPreview(filePath: downloadedPath, isVideo: isVideo, title: displayName);
    }
  }
  ...
  String? _getCachedAttachmentPath(MessageModel message, Map<String, dynamic> attachment) {
    ...
    final fileName = ChatHelpers.sanitizedFileName((attachment['name'] as String?) ?? 'attachment.bin');
  ...
  String? _getCachedVideoThumbnailPath(MessageModel message) {
    ...
    if (_attachmentCacheDirPath != null) {
      final predictedPath = '$_attachmentCacheDirPath/${message.id}_thumbnail.jpg';
  ...
  bool _shouldShowDateSeparator(int index) {
    ...
    return ChatHelpers.shouldShowDateSeparator(currentMessage.timestamp, nextMessage.timestamp);
  }
                        ...
                        children: [
                          if (showSeparator) ...[
                            ...
                            Container(
                              ...
                              child: Text(
                                ChatHelpers.formatDateSeparator(message.timestamp),
                                style: TextStyle(
                                  fontSize: 12,
                                  fontWeight: FontWeight.w600,
                          ...
                          Align(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                color: mine ? null : (isDark ? AppColors.darkSurface : AppColors.lightSurface),
            ...
            SafeArea(
              child: Container(
                ...
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    // Selected media preview - integrated in input area
                        ...
                        Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(8),
                                child: _selectedMediaType == 'video' && _selectedVideoThumbnail != null
                                    ? Stack(
                                        fit: StackFit.expand,
                                        children: [
                                          Image.memory(
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    _selectedMediaType == 'video' ? 'Video' : 'Photo',
                                    style: TextStyle(
                                      fontWeight: FontWeight.w600,
                                      fontSize: 14,
                            ...
                            IconButton(
                              onPressed: _cancelSelectedMedia,
                    ...
                    // Input row with attachment, text field, and send
                    Row(
                      children: [
                        Container(
                          width: 40,
                          ...
                          child: Container(
                            ...
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                hintText: _selectedMediaFile != null ? 'Add caption...' : 'Type a message...',
                                hintStyle: TextStyle(
                                  color: Theme.of(context).brightness == Brightness.dark
                                      ? AppColors.darkTextSecondary
                                      : AppColors.lightTextSecondary,
  ...
  Widget _buildMessageContent(MessageModel message, bool mine, bool isDark) {
    ...
    if (type == 'image' || type == 'video') {
      ...
      final videoThumbnailPath = isVideo ? _getCachedVideoThumbnailPath(message) : null;
      ...
      return Column(
        ...
        children: [
          InkWell(
            onTap: () => _onMediaMessageTap(message: message, attachment: attachment),
            borderRadius: BorderRadius.circular(12),
            child: mediaCard,
          ),
